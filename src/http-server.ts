...
app.use(paymentMiddleware(PAYMENT_WALLET as `0x${string}`, {
  '/release': {
    price: '$1.00',
 ...
 */
app.get('/status/:releaseId', async (req: Request, res: Response) => {
  try {
 ...
 */
app.get('/streams/:releaseId', async (req: Request, res: Response) => {
  try {
    const releaseId = req.params.releaseId as string;
 ...
 */
app.get('/artists', async (_req: Request, res: Response) => {
  try {
 ...
 */
app.get('/releases', async (_req: Request, res: Response) => {
  try {
