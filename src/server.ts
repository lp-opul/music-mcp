...
const audioCache: Map<string, { audio: string; image: string; title: string; timestamp: number }> = new Map();
 ...
 */
app.get('/api/audio/:taskId', (req: Request, res: Response) => {
  const taskId = req.params.taskId as string;
  const cached = audioCache.get(taskId);
...
app.post('/api/upload-track', asyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await dittoClient.createTrackWithAudioBuffer(
    cleanReleaseId,
    audioBuffer,
    `${title.replace(/[^a-zA-Z0-9]/g, '_')}.mp3`
  );
...
app.post('/api/release-full', asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    const trackResult = await dittoClient.createTrackWithAudioBuffer(
      releaseId,
      audioBuffer,
      `${trackTitle.replace(/[^a-zA-Z0-9]/g, '_')}.mp3`
    );
