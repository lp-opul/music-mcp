...
import { getArtistWallet, setArtistWallet, getWalletBalance, } from './wallet-service.js';
    ...
    try {
        switch (name) {
            ...
            case 'upload_track': {
                ...
                if (dittoClient) {
                    ...
                    if (validated.audioUrl) {
                        ...
                        result = await dittoClient.createTrackWithAudio(releaseId, validated.audioUrl, `${validated.title.replace(/[^a-zA-Z0-9]/g, '_')}.${fileExt}`);
                    }
                    else {
            ...
            case 'submit_release': {
                ...
                // Mock response
                const mockResult = createMockSubmitReleaseResponse('Release', 'Artist', ['track_1'], validated.dsps, new Date().toISOString());
                return {
                    content: [{ type: 'text', text: JSON.stringify({ ...mockResult, _mock: true }, null, 2) }],
                };
            }
            ...
            case 'get_earnings': {
                ...
                if (dittoClient) {
                    const storeId = validated.dsp ? getStoreIri(validated.dsp) : undefined;
                ...
                const mockResult = getMockEarnings(validated.releaseId, validated.trackId, validated.startDate, validated.endDate, validated.dsp);
                return {
                    content: [{ type: 'text', text: JSON.stringify({ ...mockResult, _mock: true }, null, 2) }],
                };
            }
            case 'get_streams': {
                ...
                const mockResult = getMockStreams(validated.releaseId, validated.trackId, validated.startDate, validated.endDate, validated.dsp);
                return {
                    content: [{ type: 'text', text: JSON.stringify({ ...mockResult, _mock: true }, null, 2) }],
                };
            }
            ...
            case 'set_splits': {
                ...
                if (dittoClient) {
                    const result = await dittoClient.setReleaseSplits(validated.releaseId, validated.splits);
                    return {
                        content: [{ type: 'text', text: JSON.stringify(result, null, 2) }],
                    };
                }
                const mockResult = setMockSplits(validated.releaseId, validated.splits);
                return {
                    content: [{ type: 'text', text: JSON.stringify({ ...mockResult, _mock: true }, null, 2) }],
                };
            }
            ...
            case 'release_ai_track': {
                ...
                try {
                    ...
                    const existingArtist = artistList.find((a) => a.name?.toLowerCase() === validated.artistName.toLowerCase());
                    let artistId;
                    let artistIri;
                    if (existingArtist) {
