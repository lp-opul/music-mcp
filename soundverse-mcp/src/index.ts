...
const generationResults = new Map<string, Record<string, unknown>>();
...
const tools: Tool[] = [
  {
    name: 'generate_song',
    description:
      'Generate a song from a text prompt using the Soundverse AI. Returns a generation ID that can be polled with get_generation_status.',
    inputSchema: {
      type: 'object',
      properties: {
        prompt: {
  ...
  },
  {
    ...
    description:
      'Generate a song driven primarily by provided lyrics. A style prompt can optionally refine the result.',
    inputSchema: {
      type: 'object',
      properties: {
        lyrics: {
  ...
  },
  {
    ...
    description:
      'Check the status of a previously submitted song generation. Returns progress percentage, status, and download URL when complete.',
    inputSchema: {
      type: 'object',
      properties: {
        generation_id: {
...
async function generateSong(body: Record<string, unknown>) {
  ...
  const id = (data.message_id ?? data.id ?? `sv_${Date.now()}`) as string;
...
function formatResult(data: Record<string, unknown>): string {
  const audioData = data.audioData as Array<Record<string, unknown>> | undefined;
...
const server = new Server(
  { name: 'soundverse-mcp', version: '0.1.0' },
  { capabilities: { tools: {} } },
);
  ...
  try {
    switch (name) {
      case 'generate_song': {
        ...
        const body: Record<string, string> = { prompt, StyleOfMusic: prompt };
